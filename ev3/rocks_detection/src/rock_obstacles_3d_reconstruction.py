...
class RocksDetection(object):
    ...
    def updateObjectsPose(self):
        ...
        conf = 0.50
        # get static frame
        #frame_test = '/home/pcarboni/TESP/TESP2024/resized_mirrored_image.jpeg'
        #environment_rocks = self.yolo_detection(model=self.model, frame=frame_test, class_id=self.class_dict['rock'], conf=conf)
        #environment_rovers = self.yolo_detection(model=self.model, frame=frame_test, class_id=self.class_dict['rover'], conf=conf)
        ...
        rock_positions = environment_rocks
        rover_positions = environment_rovers
        print('environment rocks', environment_rocks)
        print('rock positions', rock_positions)
        return rock_positions, rover_positions
            
if __name__ == '__main__':

    rospy.init_node('rocks_detection_node', anonymous=False)
    rock_pub = rospy.Publisher("rocks", Rocks, queue_size=10)
    ...
    while(not(rospy.is_shutdown())):
        ...
        for rock in rocks:
            rock_msg = Rock()
            rock_msg.top_x = rock[0][0]
            rock_msg.top_y = rock[0][1]
            rock_msg.bottom_x = rock[1][0]
            rock_msg.bottom_y = rock[1][1]
            rocks_msg.rocks.append(rock_msg)

        rock_pub.publish(rocks_msg)
